...
{
    QString filter = tr("html (*.htm;*.html);;csv (*.csv);;ANY FILE (*)");
    QString fname = QFileDialog::getOpenFileName(this, tr("Open"), last_dir,
                                                 filter);

    if (!fname.isEmpty()) {
        if (fname.rightRef(4) == ".htm" || fname.rightRef(5) == ".html") {
            images.clear();
